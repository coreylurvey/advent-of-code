...
data = """17-19 p: pwpzpfbrcpppjppbmppp
...
for record in records:
    ...
    character_range = columns[0]
    boundaries = character_range.split("-")
    lower_bound = int(boundaries[0]) - 1
    upper_bound = int(boundaries[1]) - 1
    character = columns[1].replace(":", "")
    password = columns[2]

    try:
        if ((password[lower_bound] == character or password[upper_bound] == character) and password[lower_bound] != password[upper_bound]):
            valid_count += 1
    except IndexError:
        pass
